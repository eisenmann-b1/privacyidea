...
class TwoStepRolloverAndVerify(MyApiTestCase):
    def test_01_rollover_and_verify_with_2step_and_pin(self):
        ...
        set_policy(
            ...
            action=["hotp_2step_difficulty=12345",
                    "hotp_2step_serversize=33",
                    "hotp_2step_clientsize=11",
                    f"{PolicyAction.VERIFY_ENROLLMENT}=hotp"],
            scope=SCOPE.ENROLL,
        )

        # Authenticate the user for the following API calls
        with self.app.test_request_context('/auth', data={"username": "cornelius",
                                                          "password": "test"},
                                           method='POST'):
            res = self.app.full_dispatch_request()
        ...
        # Create a token with PIN and assign it to a user
        tok = init_token({"type": "hotp", "otpkey": self.otpkey, "pin": "1234"},
                         user=User("cornelius",
                                   realm=self.realm1,
        ...
        with self.app.test_request_context("/validate/check", method="POST",
                                           data={"user": "cornelius",
                                                 "pass": "1234" + self.valid_otp_values[0]}):
            res = self.app.full_dispatch_request()
            assert res.status_code == 200
            result = res.json.get("result")
            assert result.get("value")

        # Start a rollover of the token
        with self.app.test_request_context('/token/init',
                                           method='POST',
                                           data={"serial": serial,
                                                 "rollover": True
        ...
        # Now doing the second part of the 2nd step ('rollover' needs to be given again)
        with self.app.test_request_context('/token/init',
                                           method='POST',
                                           data={"serial": serial,
                                                 "otpkey": hex_client_component,
        ...
        # Now we need to verify the rollover
        otpkey_bin = binascii.unhexlify(otpkey)
        otp_value = HmacOtp().generate(key=otpkey_bin, counter=1)
