        ...
        if token_type.lower() == "sshkey":
            ...
            if len(toks) == 1:
                ...
                if (user_filter and user_filter == options.get("user")) or \
                    ...
                    log.info("Using SSH key {!s} for SSH user {!s}".format(tokclass.token.serial,
                                                                              options.get("user")))
                    ...
                    log.debug("The requested user {!s} does not match the user "
                              "option ({!s}) of the SSH application.".format(
                        user_filter, options.get("user")))
        else:
            log.info(f"Token {serial!r}, type {serial!r} is not supported by "
